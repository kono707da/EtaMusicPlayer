...
- **播放列表**：客户端播放列表（跨节点聚合，存储于 eta_web 后端）+ 节点播放列表（随节点走），CRUD、系统列表「全部音乐」「收集箱」、播放列表粒度授权、m3u/m3u8 文件批量导入（保留原顺序，可选复制或剪切音乐文件入库，支持单文件或文件夹递归导入）；**播放列表文件夹**（1.5.0+）支持树形组织播放列表，m3u 文件夹导入保留相对目录层级，支持拖拽移动、右键管理
