...
from app.plugins.asmr_one.asmr_client import AsmrClient, AsmrError, DEFAULT_TIMEOUT, flatten_file_tree
from app.plugins.asmr_one.database import get_db
from app.plugins.asmr_one.downloader import (
    cancel_download_task,
    start_download_task,
)
from app.plugins.asmr_one.models import DownloadFileStatus, DownloadTask, Setting
...
DEFAULT_SETTINGS = {
    ...
    "subdir": "ASMR",  # 在 watch_dir 下的子目录
    "default_watch_dir_id": "",  # 空表示使用第一个
}
...
class VotePayload(BaseModel):
    work_id: int
    tag_id: int
    status: int = Field(ge=-1, le=1)  # 1=赞同, -1=反对, 0=取消
